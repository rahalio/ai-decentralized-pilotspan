...
Package scope: `@pilotspan` (override via `package_scope` in config when cloning).

## HARD RULE — never commit `.codegen`

`.codegen/` is **gitignored** and must **never** be committed or pushed to GitHub.
Bootstrap locally by copying from `zero-apps-codegen-scaffold`, then set `package_scope` to `@pilotspan` and run `pnpm codegen:paths`.
...
2. Set `package_scope` (e.g. `@acme`) and rename `@pilotspan/*` packages.

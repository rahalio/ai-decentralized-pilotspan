...
const DOMAIN_PATH_PATTERNS: Array<{ pattern: RegExp; domain: string }> = [
  ...
  { pattern: /\/anonymisation-gates/, domain: "anonymisation-gates" },
  { pattern: /\/dataset-approvals/, domain: "custodians" },
  { pattern: /\/quality-issues/, domain: "custodians" },
  { pattern: /\/custodians/, domain: "custodians" },
  { pattern: /\/pilots/, domain: "pilots" },
  { pattern: /\/readiness-assessments/, domain: "readiness" },
  { pattern: /\/shadow-ai-systems/, domain: "shadow-ai" },
  { pattern: /\/talent-assignments/, domain: "talent-assignments" },
];
...
export async function injectDomainDependencies(
  ...
  if (!domainDeps) {
    ...
    switch (domain) {
      case "anonymisation-gates":
        domainDeps = buildAnonymisationGatesDomainModule(dynamoClient);
        break;
      case "custodians":
        domainDeps = buildCustodiansDomainModule(dynamoClient);
        break;
      case "identity":
      ...
      case "readiness":
        ...
        break;
      case "shadow-ai":
        domainDeps = buildShadowAiDomainModule(dynamoClient);
        break;
      case "talent-assignments":
        domainDeps = buildTalentAssignmentsDomainModule(dynamoClient);
        break;
      default:
        return;
    }
...
export interface DomainDependencies {
  "anonymisation-gates": AnonymisationGatesDomainModule;
  custodians: CustodiansDomainModule;
  identity: IdentityDomainModule;
  pilots: PilotsDomainModule;
  readiness: ReadinessDomainModule;
  "shadow-ai": ShadowAiDomainModule;
  "talent-assignments": TalentAssignmentsDomainModule;
}
 ...
 */
export function getAnonymisationGatesDeps(
  request: FastifyRequest,
): DomainDependencies["anonymisation-gates"] {
  return getDomainDeps(request, "anonymisation-gates");
}
export function getCustodiansDeps(
  request: FastifyRequest,
): DomainDependencies["custodians"] {
  return getDomainDeps(request, "custodians");
}
...
export function getShadowAiDeps(
  request: FastifyRequest,
): DomainDependencies["shadow-ai"] {
  return getDomainDeps(request, "shadow-ai");
}
export function getTalentAssignmentsDeps(
  request: FastifyRequest,
): DomainDependencies["talent-assignments"] {
  return getDomainDeps(request, "talent-assignments");
}
